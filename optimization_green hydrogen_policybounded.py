...
def perform_optimization(data, q_values, p_values, n_values, i_values, market_shares, num_samples=10):
    results = []
    ...
    np.random.seed(0)  
    green_hydrogen_prices = np.random.normal(3, 0.93, num_samples)  
    ...
    for j in range(num_samples):
        ...
        renewable_electricity_tax_credit_rate = LpVariable("Renewable_Electricity_Tax_Credit_Rate", 0, 0.03)
        ...
        if sum(value(y_samples[j][i]) == 1 for i in range(512)) == 512:
            ...
            renewable_electricity_tax_credit_rate_min = LpVariable("Renewable_Electricity_Tax_Credit_Rate_Min", 0, 0.03)
            H2_CFD_rate_min = LpVariable("H2_CFD_Rate_Min", 0, 300)
            carbon_tax_rate_min = LpVariable("Carbon_Tax_Rate_Min", 0, 132)
            ...
            prob_min_support += renewable_electricity_tax_credit_rate_min * (78000) + H2_CFD_rate_min + carbon_tax_rate_min
...
def main():
    ...
    results = perform_optimization(data, q_values, p_values, n_values, i_values, market_shares)
