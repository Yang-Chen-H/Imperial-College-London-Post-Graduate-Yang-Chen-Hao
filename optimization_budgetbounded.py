...
def perform_optimization(data, p_values, n_values, i_values, max_carbon_tax, max_government_cost, market_shares, num_samples=10):
    results = []
    ...
    np.random.seed(0)  
    blue_hydrogen_prices = np.random.normal(2.16, 0.253, num_samples)  
    ...
    for j in range(num_samples):
        ...
        renewable_hydrogen_tax_credit_rate = LpVariable("Renewable_Hydrogen_Tax_Credit_Rate", 0, None)
        ...
        # Calculate total carbon tax and government budget
        total_carbon_tax = lpSum((n_values[i] if y_samples[j][i] else i_values[i]) * carbon_tax_rate for i in range(512))
        total_government_budget = lpSum(p_values * H2_CFD_rate) + lpSum(k_values  * renewable_hydrogen_tax_credit_rate)

        # Objective function to maximize blue market share 
        prob += lpSum(market_shares[i] * y_samples[j][i] for i in range(512)) 
        # Constraints to ensure the total carbon tax and government budget limits
        prob += total_carbon_tax <= max_carbon_tax
        prob += total_government_budget <= max_government_cost
        ...
        if sum(value(y_samples[j][i]) == 1 for i in range(512)) == 512:
            ...
            H2_CFD_rate_min = LpVariable("H2_CFD_Rate", 0, None)
            renewable_hydrogen_tax_credit_rate_min = LpVariable("Renewable_Hydrogen_Tax_Credit_Rate", 0, None)
            carbon_tax_rate_min = LpVariable("Carbon_Tax_Rate", 0, None)
            ...
            total_government_budget_min = lpSum(p_values * H2_CFD_rate_min) + lpSum(k_values  * renewable_hydrogen_tax_credit_rate_min)
            ...
            prob_min_support += total_government_budget_min <= max_government_cost
            ...
            results[-1]["total_government_cost"] = sum(p_values * minimal_support_params[0]) + sum(k_values  * minimal_support_params[1])
...
def main():
    ...
    max_carbon_tax = 2.5e10
    max_government_cost = 5.5e10

    # Perform optimization
    results = perform_optimization(data, p_values, n_values, i_values, max_carbon_tax, max_government_cost, market_shares)
    ...
    for result in results:
        ...
        print("Total Government Cost :", result["total_government_cost"])
